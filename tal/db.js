...
//NOTE: https://www.gstatic.com/firebasejs/3.8.0/firebase-app.js
...
//NOTE: https://www.gstatic.com/firebasejs/3.8.0/firebase-database.js
...
firebase.initializeApp({ "apiKey": "AIzaSyBykIpP661HEBT0N-dv-6KX57IbHsxx5TU", "authDomain": "tal-database.firebaseapp.com", "databaseURL": "https://tal-database.firebaseio.com", "projectId": "tal-database", "storageBucket": "tal-database.appspot.com", "messagingSenderId": "445370991826" });
...
var DB_USER_FIELD_LIST = [
	{ key: "uid", type: DB_FIELD_TYPE.STR, },
	{ key: "name", type: DB_FIELD_TYPE.STR, },
	{ key: "email", type: DB_FIELD_TYPE.STR, },
	{ key: "type", type: DB_FIELD_TYPE.STR, },
	{ key: "silver_tals", type: DB_FIELD_TYPE.UINT, },
	{ key: "copper_tals", type: DB_FIELD_TYPE.UINT, },
	{ key: "sodium_tals", type: DB_FIELD_TYPE.UINT, },
];
...
var DB_SUBKEY_LIST0 = [ "Cool", "Huge", "Teeny", "Fluffy", "Big", "Small", "Cute", "Beautiful", "Handsome", "Slender", "Adorable", "Wild", "Colossal", "IttyBitty", "Simple", "Hot", "Soft", "Bitter", "Rich", "Brave", "Clever", "Kind", "Polite", "Lucky", "Lazy", "Old", "Young", "Little", "Dark", ];
var DB_SUBKEY_LIST1 = [ "Frightened", "Talkative", "Elderly", "Capitalist", "Disenfranchised", "Displaced", "Privileged", "Vegan", "Fascist", "SelfEmployed", "Stubborn", "Diplomatic", "Tyrannical", "Exotic", "Adventurous", "Dashing", "Marital", "Judgemental", "Oppressive", "ProblemSolving", "Coughing", "Managerial", "Motivated", "Bumbling", "Burping", "Squatting", "Prancing", "Maternal", "Windswept", "Stylish", "Friendly", ];
var DB_SUBKEY_LIST2 = [ "Tomato", "Potato", "Donkey", "Sandwich", "Shoe", "Fridge", "Kitten", "Carrot", "Lamp", "Cactus", "Peach", "Dog", "Kitten", "Weasel", "Dolphin", "Hankey", "Volcano", "Donut", "Panda", ];

function db_prepare_str(val) {
	return (val !== undefined && val !== null) ? val.trim() : "";
};

function db_prepare_uint(val) {
	var uint = val | 0;
	if(uint < 0) {
		uint = 0;
	}
...
function db_prepare_record(record, field_list) {
	for(var field_index = 0; field_index < field_list.length; field_index++) {
		...
		if(field.type === DB_FIELD_TYPE.UINT) {
			record[field.key] = db_prepare_uint(record[field.key]);
		}
		else {
		...
		project_ref.once("value").then(function(project_data) {
			...
			if(project_name !== null) {
				...
				if(rev_count > 0) {
					...
					project.email = rev.child("email").val();
					project.total_silver_tals = rev.child("total_silver_tals").val() | 0;
					project.total_copper_tals = rev.child("total_copper_tals").val() | 0;
...
function db_gen_key(key_list, uid) {
	var key = uid;

	if(key_list.length < 1000) {
		var unique = false;
		while(unique === false) {
			key = "";
			key += DB_SUBKEY_LIST0[Math.floor(Math.random() * DB_SUBKEY_LIST0.length)];
			key += DB_SUBKEY_LIST1[Math.floor(Math.random() * DB_SUBKEY_LIST1.length)];
			key += DB_SUBKEY_LIST2[Math.floor(Math.random() * DB_SUBKEY_LIST2.length)];
...
function db_gen_project_keys(project, callback) {
	if(callback !== undefined) {
		var database_ref = firebase.database().ref();

		var keys_ref = database_ref.child("keys");
		keys_ref.once("value").then(function(keys_data) {
			var key_list = [];

			keys_data.forEach(function(key_data) {
				key_list[key_list.length] = key_data.key;
			});

			if(project.key.length === 0) {
				project.key = db_gen_key(key_list, project.uid);
			}
			keys_ref.child(project.key).set(project.key);

			for(var i = 0; i < project.users.length; i++) {
				var user = project.users[i];

				if(user.key.length === 0) {
					user.key = db_gen_key(key_list, user.uid);
				}
				keys_ref.child(user.key).set(user.key);
			}
		...
		if(project.rev < 0) {
			...
			db_get_project(project.key, function(stored_project) {
				...
				if(stored_project === null) {
					...
					if(project.email !== stored_project.email) {
						modified = true;
					}
					if((project.total_silver_tals | 0) !== (stored_project.total_silver_tals | 0)) {
						modified = true;
					}
					if((project.total_copper_tals | 0) !== (stored_project.total_copper_tals | 0)) {
						modified = true;
					}
...
function db_save_project(project_, callback) {
	var project = JSON.parse(JSON.stringify(project_));
	...
	project.email = db_prepare_str(project.email);
	if(project.email.length === 0) {
		errors[errors.length] = "project email is a required field.";
	}
	project.total_silver_tals = db_prepare_uint(project.total_silver_tals);
	project.total_copper_tals = db_prepare_uint(project.total_copper_tals);

	var creative_founders = 0;
	var allocated_silver_tals = 0;
	var allocated_copper_tals = 0;
	for(var user_index = 0; user_index < project.users.length; user_index++) {
		...
		allocated_silver_tals += user.silver_tals;
		allocated_copper_tals += user.copper_tals;
		//TODO: What do we do with these now??
	...
	if(allocated_silver_tals > project.total_silver_tals) {
		errors[errors.length] = allocated_silver_tals + " silver tals are allocated but only " + project.total_silver_tals + " total silver tals are available.";
	}
	if(allocated_copper_tals > project.total_copper_tals) {
		errors[errors.length] = allocated_copper_tals + " copper tals are allocated but only " + project.total_copper_tals + " total copper tals are available.";
	}
	...
	if(errors.length === 0) {
		db_gen_project_keys(project, function() {
			db_check_project_changes(project, function(modified) {
				...
				rev_ref.child("email").set(project.email);
				rev_ref.child("total_silver_tals").set(project.total_silver_tals);
				rev_ref.child("total_copper_tals").set(project.total_copper_tals);
...
function db_add_project_user(project, name, email, type, silver_tals, copper_tals, sodium_tals) {
	...
	user.email = email;
	user.type = type;
	user.silver_tals = silver_tals | 0;
	user.copper_tals = copper_tals | 0;
	user.sodium_tals = sodium_tals | 0;
...
function db_get_page_name(path) {
...
function db_get_session_project(no_redirect) {
	var project = null;
	if(window.name.length > 0) {
		//TODO: Handle parse fail!!
		project = JSON.parse(window.name);
	}

	var page_name = db_get_page_name(window.location.pathname);
	var redirect = "index.html";

	if(project != null) {
		var got_access = false;
		for(var i = 0; i < project.__pages.length; i++) {
			if(project.__pages[i] === page_name) {
				got_access = true;
				break;
			}
		}
		...
		if(got_access === true) {
			redirect = null;
			db_set_session_project(project);
		}
		else {
			if(project.__last_page !== undefined) {
				redirect = project.__last_page;
			}
		}

		// console.log(project.__last_page);
	}

	if(redirect !== null && no_redirect !== true) {
...
function db_set_session_project(project, next_page) {
	window.name = "";
	if(project !== null) {
		if(next_page !== undefined) {
			next_page = db_get_page_name(next_page);

			var in_list = false;

			if(project.__pages === undefined) {
				project.__pages = [];
			}
			else {
				for(var i = 0; i < project.__pages.length; i++) {
					if(project.__pages[i] === next_page) {
						in_list = true;
						break;
					}
				}
			...
			if(in_list === false) {
				project.__pages[project.__pages.length] = next_page;
			}
		}
		project.__last_page = db_get_page_name(window.location.pathname);

		window.name = JSON.stringify(project);
	}
};
